...
const explicitIds=process.argv.find(arg=>arg.startsWith('--ids='))?.slice(6).split(',').map(Number);
const stages=process.argv.slice(2).filter(arg=>!arg.startsWith('--')).map(Number),chosen=assetsOnly&&explicitIds?explicitIds.map(id=>({id,stageId:0})):stages.length?rows.filter(r=>stages.includes(r.stageId)):rows;
const server=await createServer({optimizeDeps:{entries:['scripts/qa/expansion-art.html']},server:{host:'127.0.0.1',port:4331,strictPort:true,watch:null,hmr:false}});await server.listen();
...
try{
 ...
 for(const row of chosen){
  const {image,animated,...record}=await page.evaluate(({id,assetsOnly})=>window.renderExpansion(id,assetsOnly),{id:row.id,assetsOnly});
  if(assetsOnly){await writeFile(`public/models/pet-${row.id}.png`,Buffer.from(image.split(',')[1],'base64'));if((row.id-321)%19===18)console.log(`Thumbnails ${row.stageId}/20`);continue;}
 ...
 if(assetsOnly){console.log('Runtime thumbnails generated; no comparisons or QA executed.');}else{
