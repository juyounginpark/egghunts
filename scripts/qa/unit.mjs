...
try {
 ...
 await test('tap limiter, damage and single hatch',()=>{const g=make();g.save.eggs=[{id:'egg',type:0,hp:30,distance:14}];g.save.selected='egg';assert.ok(g.tap());assert.equal(g.selected.hp,29);assert.equal(g.tap(),false);g.damage(1000);g.damage(1000);assert.equal(g.result,null);assert.equal(g.save.mongles.reduce((a,b)=>a+b,0),0);assert.ok(g.claimHatch('egg'));assert.equal(g.claimHatch('egg'),false);assert.equal(g.save.mongles.reduce((a,b)=>a+b,0),1);assert.equal(g.save.dust,EGGS[0].reward);});
