...
const root='artifacts/document-art',session={browser,url:'http://127.0.0.1:4324',close:async()=>{await browser.close();await server.close();}},errors=[],checks=[];
...
try{
 const page=await session.browser.newPage();page.on('pageerror',e=>errors.push(e.message));await page.goto(`${session.url}/scripts/qa/document-art.html`);await page.waitForFunction(()=>!!window.artReview);
 const mapping=JSON.parse(await readFile('docs/art/document-id-map.json','utf8'));
 ...
 for(let stage=1;stage<=20;stage++){
  const dragon=mapping.find(r=>r.stage===stage&&r.slot===10).id;
  const pair=await page.evaluate(stage=>window.artReview.eggPair(stage),stage);eggs.push(...pair.images.map(bytes));
  const sample=await page.evaluate(id=>window.artReview.specimen(id),dragon);assert.ok(sample.mismatch<1e-5,`static and rig bounds ${stage}: ${sample.mismatch}`);finishes.push(...sample.images.map(bytes));
  dragons.push(await readFile(`public/models/pet-${dragon}.png`));checks.push({stage,name:pair.name,staticRigError:sample.mismatch});console.log(`Art comparison ${stage}/20`);
 }
 ...
 for(const stage of [3,4,19]){
  await page.evaluate(({stage,ids})=>{
   window.__qa.region(stage,-34);const {game}=window.__art;
   for(const id of ids)game.save.mongles[id]=1;game.save.active=ids;game.revision++;
  },{stage,ids:mapping.filter(r=>r.stage===stage&&[0,3,10].includes(r.slot)).map(r=>r.id)});
