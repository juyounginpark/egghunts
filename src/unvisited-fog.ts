...
export class UnvisitedFog {
 ...
 private banks=Array.from({length:3},()=>{
  const material=new T.ShaderMaterial({transparent:true,depthWrite:false,side:T.DoubleSide,
   uniforms:{clock:{value:0},strength:{value:1}},
   vertexShader:'varying vec2 vUv; void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}',
   fragmentShader:`varying vec2 vUv; uniform float clock; uniform float strength;
    void main(){
     ...
     gl_FragColor=vec4(mix(vec3(.70,.76,.75),vec3(.87,.89,.82),wave),edge*wave*strength*.66);
    }`});
