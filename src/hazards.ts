import { HAZARD_BALANCE as B, STAGE_COVERS, STAGE_STEPS,ROUTE,routeStep,stagePatterns, environmentPlacement, type HazardDefinition, type Cover } from "./stage-data";
...
export class HazardManager{
 ...
 private step(dt:number,stage:number,p:HazardPlayer,hit:(h:Hazard)=>void,push:(x:number,z:number)=>void,status:(effect:string,seconds:number)=>void,secret:boolean){
  this.time+=dt;
  const defs=stagePatterns(stage);
  for(const [lane,d] of defs.entries()){
   const placement=environmentPlacement(stage,lane,p.stageOffset??0);
   const step=STAGE_STEPS[routeStep(placement.z,p.stageOffset??0,stage===20?ROUTE.finalLength:ROUTE.length)-1];
   if(Math.abs(placement.z-p.z)>36||this.time<(this.next.get(d.id)??B.spawnDelay)||this.attacks.some(h=>h.definition.id===d.id&&h.phase!=='Cooldown'))continue;
     ...
     const definition={...d,damage:Math.round(d.damage*step.damage),damagePercent:d.damagePercent*step.damage,cooldown:d.cooldown*step.cooldown,count:1,targetingType:'fixed' as const,shape:('laser solar'.split(' ').includes(d.visual)?'line':'ellipse') as HazardDefinition['shape'],radius:Math.min(2.2,d.radius),...(moving?{radius:B.movingRadius,activeDuration:B.crossingSeconds*step.cooldown}:{})};
     const h=this.spawn(definition,{...p,...placement,vx:0,vz:0});
     h.environment=true;h.angle=0;
     if(moving)h.target.x=-B.laneHalfWidth;
     h.origin={...h.target};
     this.next.set(d.id,this.time+definition.cooldown*(secret&&stage===20?B.finalSecretCooldown:1));
  }
