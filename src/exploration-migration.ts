...
import {eggAnchor,routePoint} from './exploration-route';
...
export function migrateExploration(world:WorldEgg[],bosses:Boss[]){
 for(const boss of bosses){
  const stage=boss.stageId??1,p=routePoint(stage,boss.final?.81:.72),offset=(stage-1)*48;
  boss.homeX=boss.final?3:p.x-2.6;boss.homeZ=p.z-offset;
 ...
 for(const egg of world){
  ...
  const p=egg.special?routePoint(20,.94):eggAnchor(egg.stageId,slot);
