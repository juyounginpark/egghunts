export type Peer={id:string;name:string;level?:number;isGuest?:boolean;x:number;z:number;rotation:number;appearance:number;downUntil:number;attackAt:number;hitAt?:number;velocity?:{x:number;z:number};carried:number|null;slot?:number;pets?:number[];egg?:import('./game').WorldEgg|null};
