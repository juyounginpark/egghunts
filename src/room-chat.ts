...
export class RoomChat{
 ...
 constructor(parent:HTMLElement,private stop:()=>void,send:(text:string)=>Promise<boolean>){
  ...
  this.toggle.onclick=()=>{this.stop();this.form.hidden=false;this.host.classList.add('composing');this.toggle.hidden=true;this.toggle.setAttribute('aria-expanded','true');this.input.focus();};
 ...
 close(){this.form.hidden=true;this.host.classList.remove('composing');this.input.blur();this.toggle.hidden=false;this.toggle.setAttribute('aria-expanded','false');}
 update(connected:boolean,name:string,chat:ChatMessage|null|undefined,peers:Peer[]){
  if(!connected){if(this.lastMessages.size){this.lastMessages.clear();this.log.replaceChildren();this.log.dataset.empty='true';}return;}
  const players=[{id:'self',name,chat},...peers].filter(p=>p.chat).sort((a,b)=>a.chat!.at-b.chat!.at);
  const follow=this.log.scrollHeight-this.log.scrollTop-this.log.clientHeight<24;
  let added=false;
  ...
  if(added){this.log.dataset.empty='false';if(follow)this.log.scrollTop=this.log.scrollHeight;}
 }
