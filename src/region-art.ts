...
export class RegionArt {
 ...
 private count=0;
 constructor(){this.group.add(this.ground,this.moving,this.outlines,this.movingOutlines);this.ground.receiveShadow=true;this.moving.castShadow=true;for(const m of [this.ground,this.moving,this.outlines,this.movingOutlines]){m.frustumCulled=false;m.count=0;}}
 private outline(mesh:T.InstancedMesh,index:number){
  this.dummy.scale.addScalar(MAP_OBSTACLES.outlineWidth);this.dummy.updateMatrix();mesh.setMatrixAt(index,this.dummy.matrix);
 }
